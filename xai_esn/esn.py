...
class ReservoirCell(torch.nn.Module):
    ...
    def forward(self, xt, h_prev, new_spectral_radius=None, new_input_scaling=None):
        ...
        :return: ht, ht
        """
        if new_input_scaling is None:
            input_part = torch.mm(xt, self.kernel)
        else:
            new_input_scaling = torch.mean(new_input_scaling)
            input_part = torch.mm(
                xt, new_input_scaling * self.kernel / self.input_scaling
            )

        if new_spectral_radius is None:
            state_part = torch.mm(h_prev, self.recurrent_kernel)
        else:
            new_spectral_radius = torch.mean(new_spectral_radius)
            state_part = torch.mm(
                h_prev,
                new_spectral_radius * self.recurrent_kernel / self.spectral_radius,
            )
...
class ReservoirLayer(torch.nn.Module):
    ...
    def forward(self, x, h_prev=None, new_spectral_radia=None, new_input_scalings=None):
        """Computes the output of the cell given the input and previous state.
        ...
        for t in range(x.shape[1]):
            ...
            if new_spectral_radia is None:
                new_spectral_radius = None
            else:
                new_spectral_radius = new_spectral_radia[:, t]

            if new_input_scalings is None:
                new_input_scaling = None
            else:
                new_input_scaling = new_input_scalings[:, t]

            _, h_prev = self.net(
                xt,
                h_prev,
                new_spectral_radius=new_spectral_radius,
                new_input_scaling=new_input_scaling,
            )
...
class DeepReservoir(torch.nn.Module):
    def __init__(
        ...
        assert (
            n_layers == 1
        )  # TODO: implement n_layers > 1 or use ReservoirLayer directly
    ...
    def forward(self, X, new_spectral_radia=None, new_input_scalings=None):
        ...
        for res_idx, res_layer in enumerate(self.reservoir):
            [X, h_last] = res_layer(
                X,
                new_spectral_radia=new_spectral_radia,
                new_input_scalings=new_input_scalings,
            )
            states.append(X)
